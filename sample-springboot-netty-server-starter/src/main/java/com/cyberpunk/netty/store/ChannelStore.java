package com.cyberpunk.netty.store;
...
public class ChannelStore {
     ...
     * 频道绑定 key
     */
    private static final AttributeKey<Object> CLIENT_ID = AttributeKey.valueOf("clientId");

    /**
     * 客户端业务map
     */
    private static final ConcurrentHashMap<String, String> CLIENT_INFO_MAP = new ConcurrentHashMap<>(16);

    /**
     * 客户端和频道绑定
     */
    private static final ConcurrentHashMap<String, ChannelId> CLIENT_CHANNEL_MAP = new ConcurrentHashMap<>(16);

    /**
     * 存储频道
     */
    protected static final ChannelGroup CHANNEL_GROUP = new DefaultChannelGroup(GlobalEventExecutor.INSTANCE);
    ...
    public static void closeAndClean(String clientId) {
        ...
        Optional.of(CLIENT_INFO_MAP.containsKey(clientId))
                .filter(Boolean::booleanValue)
                .ifPresent(oldValue -> CLIENT_INFO_MAP.remove(clientId));
        // 若存在旧连接，则关闭旧连接，相同clientId，不允许重复连接
        Optional.ofNullable(getChannel(clientId))
                .ifPresent(ChannelOutboundInvoker::close);
    }

    public static void putClientInfo(String clientId, String clientInfo) {
        CLIENT_INFO_MAP.put(clientId, clientInfo);
    }
