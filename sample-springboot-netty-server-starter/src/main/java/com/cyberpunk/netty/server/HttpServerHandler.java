...
import com.cyberpunk.netty.config.ServerEndpointConfig;
import com.cyberpunk.netty.matcher.WsPathMatcher;
...
class HttpServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    public HttpServerHandler(SimpleEndpointServer simpleEndpointServer, ServerEndpointConfig config, EventExecutorGroup eventExecutorGroup, boolean isCors) {
    ...
    private void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest req) {
        ...
        if (!StringUtils.isEmpty(simpleEndpointServer.getHost()) && !simpleEndpointServer.getHost().equals("0.0.0.0") && !simpleEndpointServer.getHost().equals(host.split(":")[0])) {
            if (forbiddenByteBuf != null) {
                res = new DefaultFullHttpResponse(HTTP_1_1, FORBIDDEN, forbiddenByteBuf.retainedDuplicate());
            } else {
                res = new DefaultFullHttpResponse(HTTP_1_1, FORBIDDEN);
        ...
        //path match
        String pattern = null;
        Set<WsPathMatcher> pathMatcherSet = simpleEndpointServer.getPathMatcherSet();
        ...
        if (simpleEndpointServer.hasBeforeHandshake(channel, pattern)) {
            simpleEndpointServer.doBeforeHandshake(channel, req, pattern);
            if (!channel.isActive()) {
                return;
            }
        ...
        if (handshaker == null) {
            ...
            if (config.isUseEventExecutorGroup()) {
                pipeline.addLast(eventExecutorGroup, new WebSocketServerHandler(simpleEndpointServer));
            } else {
                pipeline.addLast(new WebSocketServerHandler(simpleEndpointServer));
            }
            ...
            handshaker.handshake(channel, req).addListener(future -> {
                if (future.isSuccess()) {
                    ...
                    simpleEndpointServer.doOnOpen(channel, req, finalPattern);
                } else {
